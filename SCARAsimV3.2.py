...
l1 = float(config.get(g,'l1'))*simscale
l2 = float(config.get(g,'l2'))*simscale
...
theta2limitl = config.getint(g, 'btheta2limitl')+klimit
theta2limith = config.getint(g, 'btheta2limith')-klimit
...
while True:
    ...
    pygame.draw.line(screen, black, (int(x0), int(y0)), (int(x0 + l1 * cosd(theta1)), int(y0 + l1 * sind(theta1))), 5)

    # Draw the second link of the robot
    pygame.draw.line(screen, black, (int(x0 + l1 * cosd(theta1)), int(y0 + l1 * sind(theta1))), (int(x_end), int(y_end)), 5)
    ...
    #Checks controller toggle condition
        ...
        theta1_new = theta1 + w1
        theta2_new = theta2 + w2
        if theta1limitl <= theta1_new <= theta1limith and theta2limitl <= theta2_new <= theta2limith and abs(target_xunit)>0.05 and abs(target_yunit)>0.05:
            theta1 = theta1_new
            theta2 = theta2_new
